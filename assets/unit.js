/* ═══ product page ═══ */
/* In the single-file build the router re-runs this on the same DOM, so
   tear down the previous unit before wiring up the next one. */
...
function initUnit(ref) {
  ...
  if (window.__unitViewer) { window.__unitViewer.destroy(); window.__unitViewer = null; }
  ['#uTabs', '#uThumbs', '#uAdd', '#uView'].forEach(resetNode);
  ...
  else if (u.est && u.geo) sp.push(['الأبعاد (تقديرية)',
    u.geo.l + ' × ' + u.geo.w + ' م']);
  ...
  function showPhoto() {
    if (viewer) { viewer.destroy(); viewer = null; window.__unitViewer = null; }
  ...
  function show3D() {
    ...
    view.innerHTML =
      ...
      '<p class="u-note3d">' + (u.est
        ? 'الأبعاد غير مذكورة في بيانات المورّد، فقُدِّرت من المساحة وعدد الطوابق. '
        : 'مجسّم بمقياس رسم مبني على الأبعاد المسجّلة. ') +
        'يوضّح النسب والحجم، وليس رندراً للتشطيب النهائي.</p>' +
      '<div class="u-tools">' +
      ...
      '</div>';
    viewer = window.__unitViewer = new UnitViewer($('#uCanvas'), u.geo, u.stories,
      { pal: u.pal, cat: u.cat, arch: u.arch, glb: u.glb });
