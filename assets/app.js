...
/* pinned 3D sequence: one model, four camera moves, driven by scroll */
function initStory() {
  var track = $('#storyTrack'), cv = $('#storyCanvas');
  if (!track || !cv || !window.UnitViewer || !window.THREE) return;

  var v = new UnitViewer(cv, { shape: 'box', l: 11.5, w: 3.4, h: 3.3 }, 2, { dark: true });
  if (!v) return;
  v.spin = false;
  v.toggleDims(false);
  requestAnimationFrame(function () { v.resize(); });

  var VIEWS = ['front', 'iso', 'side', 'top'];
  var steps = $$('.story-step'), bars = $$('.story-bar i');
  var cur = -1;
  ...
  addEventListener('scroll', function () {
    ...
    requestAnimationFrame(function () {
      ...
      if (total <= 0) return;
      var p = Math.max(0, Math.min(0.999, -r.top / total));
